...
const CurrentSectionId = Context.Sections.coarptHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Coa Report'
const TitleWhenView = 'View Coa Report'
const TitleWhenEdit = 'Edit Coa Report'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('coarptHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('coarptHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('coarptHeaderEdit-btn_new', 'coarptHeader-new')
const btn_del = new $fgta5.ActionButton('coarptHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('coarptHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('coarptHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('coarptHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('coarptHeader-btn_recordstatus')
const btn_logs = document.getElementById('coarptHeader-btn_logs')
const btn_about = document.getElementById('coarptHeader-btn_about')

const frm = new $fgta5.Form('coarptHeaderEdit-frm');
const obj_coarpt_id = frm.Inputs['coarptHeaderEdit-obj_coarpt_id']
const obj_coarpt_name = frm.Inputs['coarptHeaderEdit-obj_coarpt_name']	
...
export async function init(self, args) {
	console.log('initializing coarptHeaderEdit ...')
	...
	// export async function coarptHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'coarptHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function coarptHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'coarptHeaderEdit_isEditDisabled'
		...
		// export async function coarptHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'coarptHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.coarptHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function coarptHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'coarptHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function coarptHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'coarptHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const coarptHeaderList = self.Modules.coarptHeaderList
	const listsecid = coarptHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'coarptHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function coarptHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.coarptHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'coarptHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function coarptHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'coarptHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'coarptHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function coarptHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.coarptHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.coarptHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.coarptHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.coarptHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.coarptHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'coarptHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Coa Report'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 2 Feb 2026 17:08'
