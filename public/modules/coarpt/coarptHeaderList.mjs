...
const CurrentSectionId = Context.Sections.coarptHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('coarptHeaderList-tbl')
const pnl_search = document.getElementById('coarptHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('coarptHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing coarptHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('coarptHeaderList-txt_searchtext')
		const btn_load = document.getElementById('coarptHeaderList-btn_gridload')
		...
		// coarptHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const coarptHeaderEdit = self.Modules.coarptHeaderEdit
		if (tbl.CurrentRow!=null) {
			coarptHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const coarptHeaderEdit = self.Modules.coarptHeaderEdit
	coarptHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await coarptHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, coarptHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const coarptHeaderEdit = self.Modules.coarptHeaderEdit
	coarptHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading coarptHeader data')
	console.log(params)
