...
const CurrentSectionId = Context.Sections.fflDetilList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('fflDetilList-tbl')

const btn_addrow = document.getElementById('fflDetilList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('fflDetilList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.fflHeaderEdit
	...
	// export function fflDetilList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'fflDetilList_addTableEvents'
	...
	// Extend list detil
	// export function fflDetilList_init(self) {}
	const fn_name = 'fflDetilList_init'
	const fn_fflDetilList_init = Extender[fn_name]
	if (typeof fn_fflDetilList_init === 'function') {
		fn_fflDetilList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.fflDetilList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function fflDetilList_openList(self, headerForm) {}
	const fn_name = 'fflDetilList_openList'
	const fn_fflDetilList_openList = Extender[fn_name]
	if (typeof fn_fflDetilList_openList === 'function') {
		fn_fflDetilList_openList(self, headerForm)
	}
	...
	const fflDetilEdit = self.Modules.fflDetilEdit
	const btn_addrow = fflDetilEdit.getCurrentState().Actions.newdata
...
async function openRow(self, tr) {
	...
	const fflDetilEdit = self.Modules.fflDetilEdit
	fflDetilEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await fflDetilEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, fflDetilEdit)
}
...
async function listRows(self, criteria, offset, limit, sort) {
	...
	// export async function fflDetilList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'fflDetilList_dataLoad'
...
async function deleteRows(self, data) {
	...
	try {
		...
		if (result.deleted) {
			...
			// export async function fflDetilList_rowsDeleted(self, data) {}
			const fn_name = 'fflDetilList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function fflDetilList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'fflDetilList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
