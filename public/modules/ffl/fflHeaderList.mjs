...
const CurrentSectionId = Context.Sections.fflHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('fflHeaderList-tbl')
const pnl_search = document.getElementById('fflHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('fflHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing fflHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('fflHeaderList-txt_searchtext')
		const btn_load = document.getElementById('fflHeaderList-btn_gridload')
		...
		// fflHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
async function openRow(self, tr) {
	...
	const fflHeaderEdit = self.Modules.fflHeaderEdit
	fflHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await fflHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, fflHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const fflHeaderEdit = self.Modules.fflHeaderEdit
	fflHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading fflHeader data')
	console.log(params)
