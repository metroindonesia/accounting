...
const CurrentSectionId = Context.Sections.groupPermissionEdit
...
const btn_edit = new $fgta5.ActionButton('groupPermissionEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('groupPermissionEdit-btn_save')
const btn_new = new $fgta5.ActionButton('groupPermissionEdit-btn_new', 'groupPermission-addrow')
const btn_del = new $fgta5.ActionButton('groupPermissionEdit-btn_delete', 'groupPermission-delrow')
const btn_reset = new $fgta5.ActionButton('groupPermissionEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('groupPermissionEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('groupPermissionEdit-btn_next')

const btn_recordstatus = document.getElementById('groupPermission-btn_recordstatus')
const btn_logs = document.getElementById('groupPermission-btn_logs')

const frm = new $fgta5.Form('groupPermissionEdit-frm');
const obj_grouppermission_id = frm.Inputs['groupPermissionEdit-obj_grouppermission_id']
const obj_permission_id = frm.Inputs['groupPermissionEdit-obj_permission_id']
const obj_permission_value = frm.Inputs['groupPermissionEdit-obj_permission_value']
const obj_group_id = frm.Inputs['groupPermissionEdit-obj_group_id']	
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'groupPermissionEdit_isEditDisabled'
		...
		// export function groupPermissionEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'groupPermissionEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function groupPermissionEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'groupPermissionEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function groupPermissionEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'groupPermissionEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.groupPermissionList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function groupPermissionEdit_formUnlocked(self, frm) {}
	const fn_name = 'groupPermissionEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const groupPermissionList = self.Modules.groupPermissionList
	const listsecid = groupPermissionList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(groupPermissionList.Section)
	...
	try {
		...
		// export async function groupPermissionEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'groupPermissionEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.groupPermissionList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function groupPermissionEdit_autofill(self, frm) {}
	const fn_autofill_name = 'groupPermissionEdit_autofill'
	...
	// Extender Saving
	// export async function groupPermissionEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'groupPermissionEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'groupPermissionEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function groupPermissionEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.groupPermissionList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.groupPermissionList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		// Extender Deleting
		// export async function groupPermissionEdit_dataDeleting(self, id, args) {}
		const args = { cancelDelete: false }
		const fn_datadeleting_name = 'groupPermissionEdit_dataDeleting'
		...
		// Extender Delete
		// export async function groupPermissionEdit_dataDeleted(self, data) {}
		const fn_datadeleted_name = 'groupPermissionEdit_dataDeleted'
		...
		self.Modules.groupPermissionList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.groupPermissionList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.groupPermissionList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender groupPermissionEdit_addRecordInfo
			// export async function groupPermissionEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'groupPermissionEdit_addRecordInfo'
