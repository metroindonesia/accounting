...
const CurrentSectionId = Context.Sections.groupPermissionList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('groupPermissionList-tbl')

const btn_addrow = document.getElementById('groupPermissionList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('groupPermissionList-btn_delrow')
...
export async function init(self, args) {
	...
	// export function groupPermissionList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'groupPermissionList_addTableEvents'
	...
	// Extend list detil
	// export function groupPermissionList_init(self) {}
	const fn_name = 'groupPermissionList_init'
	const fn_groupPermissionList_init = Extender[fn_name]
	if (typeof fn_groupPermissionList_init === 'function') {
		fn_groupPermissionList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.groupPermissionList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function groupPermissionList_openList(self, headerForm) {}
	const fn_name = 'groupPermissionList_openList'
	const fn_groupPermissionList_openList = Extender[fn_name]
	if (typeof fn_groupPermissionList_openList === 'function') {
		fn_groupPermissionList_openList(self, headerForm)
	}
	...
	const groupPermissionEdit = self.Modules.groupPermissionEdit
	const btn_addrow = groupPermissionEdit.getCurrentState().Actions.newdata
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			groupPermissionEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const groupPermissionEdit = self.Modules.groupPermissionEdit
	groupPermissionEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await groupPermissionEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, groupPermissionEdit)
}
...
async function listRows(self, criteria, offset, limit, sort) {
	...
	// export async function groupPermissionList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'groupPermissionList_dataLoad'
...
async function deleteRows(self, data) {
	...
	try {
		...
		if (result.deleted) {
			...
			// export async function groupPermissionList_rowsDeleted(self, data) {}
			const fn_name = 'groupPermissionList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function groupPermissionList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'groupPermissionList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
