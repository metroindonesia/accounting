import Context from './project-context.mjs'
import * as Ext from './project-ext.mjs'
...
const CurrentSectionId = Context.Sections.projectHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Project'
const TitleWhenView = 'View Project'
const TitleWhenEdit = 'Edit Project'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('projectHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('projectHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('projectHeaderEdit-btn_new', 'projectHeader-new')
const btn_del = new $fgta5.ActionButton('projectHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('projectHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('projectHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('projectHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('projectHeader-btn_recordstatus')
const btn_logs = document.getElementById('projectHeader-btn_logs')
const btn_about = document.getElementById('projectHeader-btn_about')

const frm = new $fgta5.Form('projectHeaderEdit-frm');
const obj_project_id = frm.Inputs['projectHeaderEdit-obj_project_id']
const obj_project_isdisabled = frm.Inputs['projectHeaderEdit-obj_project_isdisabled']
const obj_project_name = frm.Inputs['projectHeaderEdit-obj_project_name']
const obj_project_descr = frm.Inputs['projectHeaderEdit-obj_project_descr']	
...
export async function init(self, args) {
	console.log('initializing projectHeaderEdit ...')
	...
	// export async function projectHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'projectHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function projectHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'projectHeaderEdit_isEditDisabled'
		...
		// export async function projectHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'projectHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.projectHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function projectHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'projectHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function projectHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'projectHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const projectHeaderList = self.Modules.projectHeaderList
	const listsecid = projectHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'projectHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function projectHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.projectHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'projectHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function projectHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'projectHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'projectHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function projectHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.projectHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.projectHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.projectHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.projectHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.projectHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'projectHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Project'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 2 Feb 2026 13:04'
