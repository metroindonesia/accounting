...
const CurrentSectionId = Context.Sections.projectHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('projectHeaderList-tbl')
const pnl_search = document.getElementById('projectHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('projectHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing projectHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('projectHeaderList-txt_searchtext')
		const btn_load = document.getElementById('projectHeaderList-btn_gridload')
		...
		// projectHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const projectHeaderEdit = self.Modules.projectHeaderEdit
		if (tbl.CurrentRow!=null) {
			projectHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const projectHeaderEdit = self.Modules.projectHeaderEdit
	projectHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await projectHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, projectHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const projectHeaderEdit = self.Modules.projectHeaderEdit
	projectHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading projectHeader data')
	console.log(params)
