...
import * as Ext from './test-ext.mjs'
...
const CurrentSectionId = Context.Sections.testHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Test'
const TitleWhenView = 'View Test'
const TitleWhenEdit = 'Edit Test'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('testHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('testHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('testHeaderEdit-btn_new', 'testHeader-new')
const btn_del = new $fgta5.ActionButton('testHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('testHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('testHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('testHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('testHeader-btn_recordstatus')
const btn_logs = document.getElementById('testHeader-btn_logs')
const btn_about = document.getElementById('testHeader-btn_about')

const frm = new $fgta5.Form('testHeaderEdit-frm');
const obj_test_id = frm.Inputs['testHeaderEdit-obj_test_id']
const obj_test_nam = frm.Inputs['testHeaderEdit-obj_test_nam']	
...
export async function init(self, args) {
	console.log('initializing testHeaderEdit ...')
	...
	// export async function testHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'testHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function testHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'testHeaderEdit_isEditDisabled'
		...
		// export async function testHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'testHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.testHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function testHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'testHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function testHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'testHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const testHeaderList = self.Modules.testHeaderList
	const listsecid = testHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'testHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function testHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.testHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'testHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function testHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'testHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'testHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function testHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.testHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.testHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.testHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.testHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.testHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'testHeaderEdit_addRecordInfo'
