...
const CurrentSectionId = Context.Sections.testHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('testHeaderList-tbl')
const pnl_search = document.getElementById('testHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('testHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing testHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('testHeaderList-txt_searchtext')
		const btn_load = document.getElementById('testHeaderList-btn_gridload')
		...
		// testHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const testHeaderEdit = self.Modules.testHeaderEdit
		if (tbl.CurrentRow!=null) {
			testHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const testHeaderEdit = self.Modules.testHeaderEdit
	testHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await testHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, testHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const testHeaderEdit = self.Modules.testHeaderEdit
	testHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading testHeader data')
	console.log(params)
