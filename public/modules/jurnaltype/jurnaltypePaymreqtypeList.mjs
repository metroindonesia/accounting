...
const CurrentSectionId = Context.Sections.jurnaltypePaymreqtypeList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('jurnaltypePaymreqtypeList-tbl')

const btn_addrow = document.getElementById('jurnaltypePaymreqtypeList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('jurnaltypePaymreqtypeList-btn_delrow')
...
export async function init(self, args) {
	...
	// export function jurnaltypePaymreqtypeList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'jurnaltypePaymreqtypeList_addTableEvents'
	...
	// Extend list detil
	// export function jurnaltypePaymreqtypeList_init(self) {}
	const fn_name = 'jurnaltypePaymreqtypeList_init'
	const fn_jurnaltypePaymreqtypeList_init = Extender[fn_name]
	if (typeof fn_jurnaltypePaymreqtypeList_init === 'function') {
		fn_jurnaltypePaymreqtypeList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.jurnaltypePaymreqtypeList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function jurnaltypePaymreqtypeList_openList(self, headerForm) {}
	const fn_name = 'jurnaltypePaymreqtypeList_openList'
	const fn_jurnaltypePaymreqtypeList_openList = Extender[fn_name]
	if (typeof fn_jurnaltypePaymreqtypeList_openList === 'function') {
		fn_jurnaltypePaymreqtypeList_openList(self, headerForm)
	}
	...
	const jurnaltypePaymreqtypeEdit = self.Modules.jurnaltypePaymreqtypeEdit
	const btn_addrow = jurnaltypePaymreqtypeEdit.getCurrentState().Actions.newdata
	const btn_edit = jurnaltypePaymreqtypeEdit.getCurrentState().Actions.edit
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			jurnaltypePaymreqtypeEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const jurnaltypePaymreqtypeEdit = self.Modules.jurnaltypePaymreqtypeEdit
	jurnaltypePaymreqtypeEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await jurnaltypePaymreqtypeEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, jurnaltypePaymreqtypeEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/paymreqtype-list`
	const evt = { url, limit }

	// export async function jurnaltypePaymreqtypeList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'jurnaltypePaymreqtypeList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/paymreqtype-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function jurnaltypePaymreqtypeList_rowsDeleted(self, data) {}
			const fn_name = 'jurnaltypePaymreqtypeList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function jurnaltypePaymreqtypeList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'jurnaltypePaymreqtypeList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
