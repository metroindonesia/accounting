...
const CurrentSectionId = Context.Sections.jurnalmodelHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('jurnalmodelHeaderList-tbl')
const pnl_search = document.getElementById('jurnalmodelHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('jurnalmodelHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing jurnalmodelHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('jurnalmodelHeaderList-txt_searchtext')
		const btn_load = document.getElementById('jurnalmodelHeaderList-btn_gridload')
		...
		// jurnalmodelHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const jurnalmodelHeaderEdit = self.Modules.jurnalmodelHeaderEdit
		if (tbl.CurrentRow!=null) {
			jurnalmodelHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const jurnalmodelHeaderEdit = self.Modules.jurnalmodelHeaderEdit
	jurnalmodelHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await jurnalmodelHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, jurnalmodelHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const jurnalmodelHeaderEdit = self.Modules.jurnalmodelHeaderEdit
	jurnalmodelHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading jurnalmodelHeader data')
	console.log(params)
