...
const CurrentSectionId = Context.Sections.txrowtypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('txrowtypeHeaderList-tbl')
const pnl_search = document.getElementById('txrowtypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('txrowtypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing txrowtypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('txrowtypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('txrowtypeHeaderList-btn_gridload')
		...
		// txrowtypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const txrowtypeHeaderEdit = self.Modules.txrowtypeHeaderEdit
		if (tbl.CurrentRow!=null) {
			txrowtypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const txrowtypeHeaderEdit = self.Modules.txrowtypeHeaderEdit
	txrowtypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await txrowtypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, txrowtypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const txrowtypeHeaderEdit = self.Modules.txrowtypeHeaderEdit
	txrowtypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading txrowtypeHeader data')
	console.log(params)
