...
const CurrentSectionId = Context.Sections.poHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('poHeaderList-tbl')
const pnl_search = document.getElementById('poHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('poHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing poHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('poHeaderList-txt_searchtext')
		const btn_load = document.getElementById('poHeaderList-btn_gridload')
		...
		// poHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const poHeaderEdit = self.Modules.poHeaderEdit
		if (tbl.CurrentRow!=null) {
			poHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const poHeaderEdit = self.Modules.poHeaderEdit
	poHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await poHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, poHeaderEdit)
...
async function tbl_loadData(self, params={}) {
	console.log('loading poHeader data')
	console.log(params)
