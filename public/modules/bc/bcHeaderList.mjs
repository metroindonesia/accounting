...
const CurrentSectionId = Context.Sections.bcHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('bcHeaderList-tbl')
const pnl_search = document.getElementById('bcHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('bcHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing bcHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('bcHeaderList-txt_searchtext')
		const btn_load = document.getElementById('bcHeaderList-btn_gridload')
		...
		// bcHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const bcHeaderEdit = self.Modules.bcHeaderEdit
		if (tbl.CurrentRow!=null) {
			bcHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const bcHeaderEdit = self.Modules.bcHeaderEdit
	bcHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await bcHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, bcHeaderEdit)
...
async function tbl_loadData(self, params={}) {
	console.log('loading bcHeader data')
	console.log(params)
