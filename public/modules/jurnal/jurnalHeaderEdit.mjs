...
const obj_jurnal_version = frm.Inputs['jurnalHeaderEdit-obj_jurnal_version']
...
const obj_paymreq_id = frm.Inputs['jurnalHeaderEdit-obj_paymreq_id']
...
const obj_struct_id = frm.Inputs['jurnalHeaderEdit-obj_struct_id']
...
export async function init(self, args) {
	...
	// Combobox: obj_paymreq_id
	obj_paymreq_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_paymreq_id_selected'
		const fn_selected = Extender[fn_selected_name]
		if (typeof fn_selected === 'function') {
			// create function di Extender:
			// export async function obj_paymreq_id_selected(self, obj_paymreq_id, frm, evt) {}
			fn_selected(self, obj_paymreq_id, frm, evt)
		} else {	
			console.warn('Extender.obj_paymreq_id_selected is not implemented')
		}		
	})
	
	obj_paymreq_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_paymreq_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_paymreq_id_selecting(self, obj_paymreq_id, frm, evt) {}
			fn_selecting(self, obj_paymreq_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'paymreq/header-list'
			const sort = {}
			const criteria = {
				searchtext: searchtext,
			}
			...
			const fn_selecting_criteria_name = 'obj_paymreq_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_paymreq_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.paymreq_id, row.paymreq_descr, row)
				}
	...
	obj_paymreq_id.addEventListener('populating', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_populating_name = 'obj_paymreq_id_populating'
		...
		if (typeof fn_populating === 'function') {
			...
			fn_populating(self, obj_paymreq_id, frm, evt)
		} else {	
			console.warn('Extender.obj_paymreq_id_populating is not implemented')
		}		
	})
	
	
	// Combobox: obj_periode_id
	obj_periode_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_periode_id_selected'
		const fn_selected = Extender[fn_selected_name]
		if (typeof fn_selected === 'function') {
			// create function di Extender:
			// export async function obj_periode_id_selected(self, obj_periode_id, frm, evt) {}
			fn_selected(self, obj_periode_id, frm, evt)
		} else {	
			console.warn('Extender.obj_periode_id_selected is not implemented')
		}		
	})
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 23 Feb 2026 16:19'
