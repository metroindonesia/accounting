...
const Extender = Ext.extenderRef ?? Ext


const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.partnerRefEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source
const CurrentState = {}

const TitleWhenNew = 'New Reference'
const TitleWhenView = 'View Reference'
const TitleWhenEdit = 'Edit Reference'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('partnerRefEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('partnerRefEdit-btn_save')
const btn_new = new $fgta5.ActionButton('partnerRefEdit-btn_new', 'partnerRef-addrow')
const btn_del = new $fgta5.ActionButton('partnerRefEdit-btn_delete', 'partnerRef-delrow')
const btn_reset = new $fgta5.ActionButton('partnerRefEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('partnerRefEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('partnerRefEdit-btn_next')

const btn_recordstatus = document.getElementById('partnerRef-btn_recordstatus')
const btn_logs = document.getElementById('partnerRef-btn_logs')
...
export async function init(self, args) {
	...
	// Combobox: obj_interface_id
	obj_interface_id.addEventListener('selecting', async (evt)=>{
		const fn_selecting_name = 'obj_interface_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_interface_id_selecting(self, obj_interface_id, frm, evt) {}
			fn_selecting(self, obj_interface_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'interface/header-list'
			const sort = {}
			const criteria = {
			...
			// export function obj_interface_id_selecting_criteria(self, obj_interface_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_interface_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_interface_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.interface_id, row.interface_name, row)
				}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'partnerRefEdit_isEditDisabled'
		...
		// export function partnerRefEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'partnerRefEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function partnerRefEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'partnerRefEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function partnerRefEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'partnerRefEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/ref-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/ref-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/ref-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/ref-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.partnerRefList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function partnerRefEdit_formUnlocked(self, frm) {}
	const fn_name = 'partnerRefEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const partnerRefList = self.Modules.partnerRefList
	const listsecid = partnerRefList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(partnerRefList.Section)
	...
	try {
		...
		// export async function partnerRefEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'partnerRefEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.partnerRefList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function partnerRefEdit_autofill(self, frm) {}
	const fn_autofill_name = 'partnerRefEdit_autofill'
	...
	// Extender Saving
	// export async function partnerRefEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'partnerRefEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'partnerRefEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function partnerRefEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.partnerRefList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.partnerRefList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		// Extender Deleting
		// export async function partnerRefEdit_dataDeleting(self, id, args) {}
		const args = { cancelDelete: false }
		const fn_datadeleting_name = 'partnerRefEdit_dataDeleting'
		...
		// Extender Delete
		// export async function partnerRefEdit_dataDeleted(self, data) {}
		const fn_datadeleted_name = 'partnerRefEdit_dataDeleted'
		...
		self.Modules.partnerRefList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.partnerRefList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.partnerRefList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender partnerRefEdit_addRecordInfo
			// export async function partnerRefEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'partnerRefEdit_addRecordInfo'
