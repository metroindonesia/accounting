...
const CurrentSectionId = Context.Sections.partnerRefList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('partnerRefList-tbl')

const btn_addrow = document.getElementById('partnerRefList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('partnerRefList-btn_delrow')
...
export async function init(self, args) {
	...
	// export function partnerRefList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'partnerRefList_addTableEvents'
	...
	// Extend list detil
	// export function partnerRefList_init(self) {}
	const fn_name = 'partnerRefList_init'
	const fn_partnerRefList_init = Extender[fn_name]
	if (typeof fn_partnerRefList_init === 'function') {
		fn_partnerRefList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.partnerRefList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function partnerRefList_openList(self, headerForm) {}
	const fn_name = 'partnerRefList_openList'
	const fn_partnerRefList_openList = Extender[fn_name]
	if (typeof fn_partnerRefList_openList === 'function') {
		fn_partnerRefList_openList(self, headerForm)
	}
	...
	const partnerRefEdit = self.Modules.partnerRefEdit
	const btn_addrow = partnerRefEdit.getCurrentState().Actions.newdata
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			partnerRefEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const partnerRefEdit = self.Modules.partnerRefEdit
	partnerRefEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await partnerRefEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, partnerRefEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/ref-list`
	const evt = { url, limit }

	// export async function partnerRefList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'partnerRefList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/ref-delete-rows`
	try {
		
		const result = await Module.apiCall(url, { data }) 
		if (result.deleted) {

			// export async function partnerRefList_rowsDeleted(self, data) {}
			const fn_name = 'partnerRefList_rowsDeleted'
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export async function partnerRefList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'partnerRefList_tableDataLoaded'
		const fn = Extender[fn_name]
		if (typeof fn === 'function') {
