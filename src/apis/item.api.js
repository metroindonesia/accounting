...
import * as Extender from './extenders/item.apiext.js'

const moduleName = 'item'
const headerSectionName = 'header'
const headerTableName = 'public.item' 
const headerPrimaryKey = 'item_id' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await item_init(this, body) }

	// extender call
	async execute(body) { return await item_execute(this, body) }

	// header
	async headerList(body) { return await item_headerList(this, body) }
	async headerOpen(body) { return await item_headerOpen(this, body) }
	async headerUpdate(body) { return await item_headerUpdate(this, body)}
	async headerCreate(body) { return await item_headerCreate(this, body)}
	async headerDelete(body) { return await item_headerDelete(this, body) }

			
}	

// init module
async function item_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.item_init === 'function') {
			// export async function item_init(self, initialData) {}
			await Extender.item_init(self, initialData)
		}
...
async function item_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, item_log) {}
		return await Extender[fnName](self, db, body, item_log)
	} else {
...
// data logging
async function item_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function item_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `item_name ILIKE '%' || \${searchtext} || '%'`,
	};
	...
	try {
		...
		for (var row of rows) {
			...
			// lookup: struct_name dari field struct_name pada table public.struct dimana (public.struct.struct_id = public.item.struct_id)
			{
				const { struct_name } = await sqlUtil.lookupdb(db, 'public.struct', 'struct_id', row.struct_id)
				row.struct_name = struct_name
			}
			// lookup: itemclass_name dari field itemclass_name pada table public.itemclass dimana (public.itemclass.itemclass_id = public.item.itemclass_id)
			{
...
async function item_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { item_id: id }
		const searchMap = { item_id: `item_id = \${item_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function item_headerCreate(self, body) {
	const { source='item', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			const cmd = sqlUtil.createInsertCommand(tablename, data, ['item_id'])
			...
			// record log
			item_log(self, body, startTime, tablename, ret.item_id, 'CREATE', logMetadata)
...
async function item_headerUpdate(self, body) {
	const { source='item', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['item_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			item_log(self, body, startTime, tablename, data.item_id, 'UPDATE')
...
async function item_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['item_id'])
			...
			// record log
			item_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
