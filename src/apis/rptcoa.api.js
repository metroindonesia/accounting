...
import notifyClient from '@agung_dhewe/webapps/src/notifier.js'
...
async function reportviewer_fetch(self, body) {
	...
	try {
		const sql = `
			select
				rowid,
				coa_id, coa_name, coa_level,
				curr_name, agingtype_name,
				isgroup, iscoa, isbold, isitalic
			from temp.coahierarchy
...
export const runDetachedWorker = (notifierServer, clientId, options) => {
	...
	const workerPath = path.join(context.getRootDirectory(), 'src', 'workers', 'rptcoa.worker.js')
