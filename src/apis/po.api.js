...
import * as Extender from './extenders/po.apiext.js'

const moduleName = 'po'
const headerSectionName = 'header'
const headerTableName = 'public.po' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await po_init(this, body) }

	// extender call
	async execute(body) { return await po_execute(this, body) }

	// header
	async headerList(body) { return await po_headerList(this, body) }
	async headerOpen(body) { return await po_headerOpen(this, body) }
	async headerUpdate(body) { return await po_headerUpdate(this, body)}
	async headerCreate(body) { return await po_headerCreate(this, body)}
	async headerDelete(body) { return await po_headerDelete(this, body) }

			
}	

// init module
async function po_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.po_init === 'function') {
			// export async function po_init(self, initialData) {}
			await Extender.po_init(self, initialData)
		}
...
async function po_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, po_log) {}
		return await Extender[fnName](self, db, body, po_log)
	} else {
...
// data logging
async function po_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function po_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `po_doc = \${searchtext} OR po_descr ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function po_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { po_id: id }
		const searchMap = { po_id: `po_id = \${po_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function po_headerCreate(self, body) {
	const { source='po', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			const args = { section: 'header', doc_id:'ORDR' }
			...
			// generate data sesuai prefix dari doc_id (default:  ORDR) reset pertahun
			const seqdata = await sequencer.yearly(args.doc_id)	
			data.po_id = seqdata.id
			...
			// record log
			po_log(self, body, startTime, tablename, ret.po_id, 'CREATE', logMetadata)
...
async function po_headerUpdate(self, body) {
	const { source='po', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['po_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			po_log(self, body, startTime, tablename, data.po_id, 'UPDATE')
...
async function po_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['po_id'])
			...
			// record log
			po_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
