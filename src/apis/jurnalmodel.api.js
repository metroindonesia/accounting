...
import * as Extender from './extenders/jurnalmodel.apiext.js'

const moduleName = 'jurnalmodel'
const headerSectionName = 'header'
const headerTableName = 'public.jurnalmodel' 	
...
export default class extends Api {
	...
	async headerOpen(body) { return await jurnalmodel_headerOpen(this, body) }
	async headerUpdate(body) { return await jurnalmodel_headerUpdate(this, body)}
	async headerCreate(body) { return await jurnalmodel_headerCreate(this, body)}
	async headerDelete(body) { return await jurnalmodel_headerDelete(this, body) }

			
}	

// init module
async function jurnalmodel_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.jurnalmodel_init === 'function') {
			// export async function jurnalmodel_init(self, initialData) {}
			await Extender.jurnalmodel_init(self, initialData)
		}
...
async function jurnalmodel_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, jurnalmodel_log) {}
		return await Extender[fnName](self, db, body, jurnalmodel_log)
	} else {
...
// data logging
async function jurnalmodel_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function jurnalmodel_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `jurnalmodel_name = \${searchtext}`,
	};
...
async function jurnalmodel_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { jurnalmodel_id: id }
		const searchMap = { jurnalmodel_id: `jurnalmodel_id = \${jurnalmodel_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function jurnalmodel_headerCreate(self, body) {
	const { source='jurnalmodel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			jurnalmodel_log(self, body, startTime, tablename, ret.jurnalmodel_id, 'CREATE', logMetadata)
...
async function jurnalmodel_headerUpdate(self, body) {
	const { source='jurnalmodel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['jurnalmodel_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			jurnalmodel_log(self, body, startTime, tablename, data.jurnalmodel_id, 'UPDATE')
...
async function jurnalmodel_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {jurnalmodel_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['jurnalmodel_id'])
			...
			// record log
			jurnalmodel_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
