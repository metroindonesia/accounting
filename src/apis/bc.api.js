...
const headerTableName = 'public.bc' 	
...
export default class extends Api {
	...
	// extender call
	async execute(body) { return await bc_execute(this, body) }
...
async function bc_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, bc_log) {}
		return await Extender[fnName](self, db, body, bc_log)
	} else {
...
async function bc_headerList(self, body) {
	...
	const searchMap = {
		searchtext: `bc_doc = \${searchtext} OR bc_descr ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function bc_headerCreate(self, body) {
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			const args = { section: 'header', doc_id:'BCSE' }
			...
			// generate data sesuai prefix dari doc_id (default:  BCSE) reset pertahun
