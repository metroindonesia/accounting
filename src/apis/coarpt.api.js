...
import * as Extender from './extenders/coarpt.apiext.js'

const moduleName = 'coarpt'
const headerSectionName = 'header'
const headerTableName = 'public.coarpt' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await coarpt_init(this, body) }

	// extender call
	async execute(body) { return await coarpt_execute(this, body) }

	// header
	async headerList(body) { return await coarpt_headerList(this, body) }
	async headerOpen(body) { return await coarpt_headerOpen(this, body) }
	async headerUpdate(body) { return await coarpt_headerUpdate(this, body)}
	async headerCreate(body) { return await coarpt_headerCreate(this, body)}
	async headerDelete(body) { return await coarpt_headerDelete(this, body) }

			
}	

// init module
async function coarpt_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.coarpt_init === 'function') {
			// export async function coarpt_init(self, initialData) {}
			await Extender.coarpt_init(self, initialData)
		}
...
async function coarpt_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, coarpt_log) {}
		return await Extender[fnName](self, db, body, coarpt_log)
	} else {
...
// data logging
async function coarpt_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function coarpt_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `coarpt_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function coarpt_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { coarpt_id: id }
		const searchMap = { coarpt_id: `coarpt_id = \${coarpt_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function coarpt_headerCreate(self, body) {
	const { source='coarpt', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			coarpt_log(self, body, startTime, tablename, ret.coarpt_id, 'CREATE', logMetadata)
...
async function coarpt_headerUpdate(self, body) {
	const { source='coarpt', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['coarpt_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			coarpt_log(self, body, startTime, tablename, data.coarpt_id, 'UPDATE')
...
async function coarpt_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {coarpt_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['coarpt_id'])
			...
			// record log
			coarpt_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
