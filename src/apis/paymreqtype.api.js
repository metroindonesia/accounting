...
const headerTableName = 'public.paymreqtype' 	
...
export default class extends Api {
	...
	// extender call
	async execute(body) { return await paymreqtype_execute(this, body) }
...
async function paymreqtype_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, paymreqtype_log) {}
		return await Extender[fnName](self, db, body, paymreqtype_log)
	} else {
...
async function paymreqtype_headerList(self, body) {
	...
	try {
		...
		for (var row of rows) {
			...
			// lookup: agingtype_name dari field agingtype_name pada table public.agingtype dimana (public.agingtype.agingtype_id = public.paymreqtype.agingtype_id)
			{
