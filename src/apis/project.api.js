...
import * as Extender from './extenders/project.apiext.js'
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await project_init(this, body) }

	// extender call
	async execute(body) { return await project_execute(this, body) }

	// header
	async headerList(body) { return await project_headerList(this, body) }
	async headerOpen(body) { return await project_headerOpen(this, body) }
	async headerUpdate(body) { return await project_headerUpdate(this, body)}
	async headerCreate(body) { return await project_headerCreate(this, body)}
	async headerDelete(body) { return await project_headerDelete(this, body) }

			
}	

// init module
async function project_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.project_init === 'function') {
			// export async function project_init(self, initialData) {}
			await Extender.project_init(self, initialData)
		}
...
async function project_execute(self, body) {
	const { fnName } = body
	...
	if (typeof Extender[fnName] === 'function') {
		// export async function [fnName](self, db, body, project_log) {}
		return await Extender[fnName](self, db, body, project_log)
	} else {
...
// data logging
async function project_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function project_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `project_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function project_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { project_id: id }
		const searchMap = { project_id: `project_id = \${project_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		const sql = sqlUtil.createSqlSelect({
			tablename: tablename, 
			columns:[], 
...
async function project_headerCreate(self, body) {
	const { source='project', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			const args = { section: 'header', doc_id:'PROJ' }
			...
			// generate short id sesuai prefix (default: PROJ) reset pertahun
			const seqdata = await sequencer.yearlyshort(args.prefix)
			data.project_id = seqdata.id
			...
			// record log
			project_log(self, body, startTime, tablename, ret.project_id, 'CREATE', logMetadata)
...
async function project_headerUpdate(self, body) {
	const { source='project', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['project_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			project_log(self, body, startTime, tablename, data.project_id, 'UPDATE')
...
async function project_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {project_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['project_id'])
			...
			// record log
			project_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
